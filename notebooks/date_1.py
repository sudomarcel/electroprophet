...
def get_season(df_scaled: pd.DataFrame) -> np.ndarray:
    ...
    df_scaled.join(pd.get_dummies(df_scaled['season'], prefix='season'))
...
def get_weekday(df_scaled: pd.DataFrame) -> np.ndarray:
    ...
    df_scaled.join(pd.get_dummies(df_scaled['weekday'], prefix='weekday'))
...
def get_period_day(df_scaled: pd.DataFrame) -> pd.DataFrame:
    ...
    df_scaled.join(pd.get_dummies(df_scaled['period'], prefix='period'))
    df_scaled.drop('period', axis=1, inplace=True)
    return df_scaled